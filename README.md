...
制作候補・夜タスクログ・制作リンク・Quality Gate を核に、**12カード**を1画面へ縦に積んだ、しずく専用の個人ダッシュボード。スマホ幅を基準にした1カラム（最大400px程度・320pxまで破綻しない）、水色×白×薄グレーの静かなUI。

> **公開中：v0.19・12カード**（2026-08-31 実測：公開URLで配信中のCSSに v0.19 の文字色 `#5C697D` が含まれることを確認）。
> ローカルの `main` と公開URLは同期済み。
...
> スクリーンショットは 2026-08-30 に差し替え済み。12カード全体（ページ全体）を撮影しています。
...
## 変更履歴
- **v0.19（公開中）**：表記の事実整合。AI Role Router のモデル名を正本（`.claude/rules/model-usage.md`・2026-08-25）へ更新（Opus 4.8 → **Opus 5**／Sonnet 4.6 → **Sonnet 5**）。あわせて README・RELEASE_LOG・各下書きにあった「公開URLへ未反映」の記述を、実測（公開中のビルドと `origin/main` のビルド結果が一致）に合わせて訂正。ROADMAP・PORTFOLIO_NOTE の古い記載も実態へ。あわせて**二次テキストのコントラストを AA へ**（`neutral2` に文字専用の `500 = #5C697D` を追加し47か所を差し替え。最小 4.54:1・不合格0件。面・背景・枠線の色は不変）。**機能追加・UI構造の変更なし。**
- **v0.18**：「今日の次アクション」カードを追加して**12カード**に（迷ったらこれ1つ・上部固定・完了チェック）。Figma準拠。
