...
> **現在の実装：v0.18・12カード。** 公開URLのデプロイはまだ（反映は次回デプロイ後）。
...
## Roadmap
v0.18 では、12カード・折りたたみ・週次振り返りまで実装済み。外部API接続とn8n本接続は未実装。→ [`docs/ROADMAP.md`](./docs/ROADMAP.md)

---

## 変更履歴
- **v0.18（未デプロイ）**：「今日の次アクション」カードを追加して**12カード**に（迷ったらこれ1つ・上部固定・完了チェック）。Figma準拠。
...
外部API接続・n8n本接続など、v0.18 で未実装の候補は [`docs/ROADMAP.md`](./docs/ROADMAP.md) で管理します。
